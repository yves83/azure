...
then 
    ...
    echo -e "Usage: \n$0 {SubscriptionID} {ResourceGroup} {WorkspaceName} {RetentionTime}"
    exit 1
fi
...
do
   ...
   az monitor log-analytics workspace table update --subscription $subkey --resource-group $rg --workspace-name $ws --name $n --retention-time $trt --no-wait 
done
